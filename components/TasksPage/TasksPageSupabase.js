  ...
  if (updateDictCopy["habitHistory"] != null) {
    ...
    for (const entry of updateDictCopy["habitHistory"]) {
      newhabitHistory.push({
        ...entry,
        exactDueDate: entry["exactDueDate"].toISOString(),
      });
    }
...
export const supabaseInsertHabitHistoryEntries = async (
  ...
  for (let i = 0; i < entriesToAdd.length; i += batchSize) {
    ...
    const { error } = await supabase.from("HabitHistory").insert(batch);
...
// local AND supabase changes
export const supabaseInsertTask = async (
  session,
  newTaskSetting,
  setTaskItems,
  taskItems,
  ...
  for (const task of data) {
    ...
    if (task.habitHistory != null) {
      ...
      for (const entry of task["habitHistory"]) {
        newhabitHistory.push({
          ...entry,
          exactDueDate: new Date(entry.exactDueDate),
        });
      }
...
export const supabaseFixHistoryForSingleHabit = async (
  ...
  for (var i = 0; i < datesToCheck.length; i += 1) {
    ...
    // if entry does exist for this date (given bcuz else if)
      ...
      selectedEntry["status"] == "pending" && // & is "pending"
      ...
      updateEntries.push({
        status: "incomplete",
        id: selectedEntry["id"],
        habit_due_date: selectedEntry["habit_due_date"],
      });
    }
  }
 ...
 */
export const editSelectedHabitOn_ConfirmEdit = async ({
  ...
  updateDict = getHabitHistoryUpdateDict({
    initialHabitSettings,
    habitSettingsEdited,
  });
...
export const editSelectedAndUpcoming_OnConfirmEdit = async ({
  ...
  const updateDict = getHabitHistoryUpdateDict({
    initialHabitSettings,
    habitSettingsEdited,
  });
  ...
  if (startIndex == -1) {
    console.warn(
      f`editSelectedAndUpcoming_OnConfirmEdit: unable to find habit history entry with habit_due_date of ${initialHabitHistoryEntry.habit_due_date}`
    );
  }
  ...
  for (let i = startIndex; i < allHabitEntries.length; i++) {
    ...
    await supabaseUpdateHabitHistoryEntry(
      ...
      habitHistory,
      setHabitHistory,
      selectedHabitHistoryEntry.habit_due_date,
      setHabitStats
    );
 ...
 * @param {ReactState} setLoadingString loading string that gets displayed while process is loading
 */
export const editAll_OnConfirmEdit = async ({
  session,
  initialHabitSettings,
  habitSettingsEdited,
  initialHabitHistoryEntry,
