    ...
    const updateSession = async () => {
      supabase.auth.getSession().then(async ({ data: { session } }) => {
        ...
        if (session && session.user) {
          ...
          const newData = await supabaseSyncLocalWithDb(session, setTaskItems, setHabitStats, setHabitHistory, setRewardItems);
          console.log("synced");
    ...
    <>
      {session && session.user ? (
        <>
          ...
          {currentPage != "enter_new_password_form" && <>{myNavBar}</>}
        </>
      ) : (
        <>
